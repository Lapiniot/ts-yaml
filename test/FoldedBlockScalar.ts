...
suite("Folded block scalar parsing", () => {
...
`);
    ...
    test("Parse folded block scalar, detect content indentation", () => {
        const actual = Parser.parse(String.raw`>
 folded
 text`);
        const expected = "folded text\n";
        assert.equal(actual, expected);
    });

    test("Parse folded block scalar, detect content indentation, do not fold more indented lines", () => {
        const actual = Parser.parse(String.raw`>
 folded
   text
     content`);
        const expected = "folded\n  text\n    content\n";
        assert.equal(actual, expected);
    });

    test("Parse folded block scalar, normalize line breaks to LF", () => {
        const actual = Parser.parse(">\r\n\r\nfolded\r\n\r\ntext\r\rcontent\n\n");
        const expected = "\nfolded\ntext\ncontent\n";
        assert.equal(actual, expected);
    });

    test("Parse folded block scalar, allow optional one-line comment in the header after indicators", () => {
    ...
    test("Parse folded block scalar, don't parse comment in the content (include text as-is)", () => {
        const actual = Parser.parse(String.raw`>
      folded # some comment
      text
      # another comment`);
        const expected = "folded # some comment text # another comment\n";
        assert.equal(actual, expected);
    });

    test("Parse folded block scalar, with valid content indentation indicator 1 through 9", async ctx => {
        ctx.plan(9);
        for (let i = 1; i <= 9; i++) {
...
${" ".repeat(i)}  text`);
    ...
    test("Throw an error for invalid comment in the header after indicators without WS separation", () => {
     ...
     folded
     text`), error => error instanceof Error && error.message === "Invalid folded block scalar at (Ln 1, Col 1).");
    });
    ...
    test("Parse folded block scalar with leading empty lines", () => {
        const actual = Parser.parse(String.raw`>
    ...
    test("Parse folded block scalar with content (including WS only) and empty lines interleaved", () => {
        const actual = Parser.parse(String.raw`>
    ...
    test("Parse folded block scalar with trailing empty lines (implicit CLIP chomping)", () => {
        const actual = Parser.parse(String.raw`>
        folded
        text
    ...
    test("Parse folded block scalar with trailing empty lines (explicit STRIP chomping)", () => {
        const actual = Parser.parse(String.raw`>-
        folded
        text
    ...
    test("Parse folded block scalar with trailing empty lines (explicit KEEP chomping)", () => {
        const actual = Parser.parse(String.raw`>+
        folded
        text
    ...
    test("Parse folded block scalar with both chomping method and content indentation indicators", async ctx => {
        ctx.plan(4);
